...
class AutoFireGUI:
    def __init__(self):
        ...
        self.fire_rate = 5
        self.rate_mode = "cps"  # "cps" oder "interval"
        self.delay_threshold = 0.0

        self.root = tk.Tk()
        self.root.title("BUTOON IT!")
        ...
        # Status mit Flicker-Hintergrund (oben statt Header)
        status_frame = tk.Frame(main, bg="#0d0d0d")
        status_frame.pack(pady=2)

        self.flicker = tk.Canvas(status_frame, width=120, height=20, bg="#000000", highlightthickness=0)
        self.flicker.pack()
        
        self.status_label = tk.Label(status_frame, text="BUTTOON IT!", font=("Consolas", 10, "bold"), 
                                     fg="#00ff41", bg="#000000")
        self.status_label.place(in_=self.flicker, relx=0.5, rely=0.5, anchor="center")

        self.select_btn = tk.Button(main, text="[ BUTTON ]", command=self.start_key_selection,
                                    font=("Consolas", 9, "bold"), fg="#00ff41", bg="#1a1a1a",
                                    relief='flat', cursor='hand2', padx=8, pady=3)
        self.select_btn.pack(pady=2)

        self.key_label = tk.Label(main, text="[ NO BUTTON ]", font=("Consolas", 9, "bold"), fg="#00cc33", bg="#0d0d0d")
        self.key_label.pack(pady=2)

        # Status mit Flicker-Hintergrund
        status_frame = tk.Frame(main, bg="#0d0d0d")
        status_frame.pack(pady=2)
        ...
        self.status_label = tk.Label(status_frame, text="BUTTOONED!", font=("Consolas", 10, "bold"), 
                                     fg="#00ff41", bg="#000000")
        self.status_label.place(in_=self.flicker, relx=0.5, rely=0.5, anchor="center")
        ...
        # Container für beide Regler (an gleicher Position)
        ...
        # Flicker Pixels erstellen (unsichtbar am Anfang)
        self.flicker_pixels = []
        for i in range(60):
            x = (i % 30) * 4
            y = (i // 30) * 10
            pixel = self.flicker.create_rectangle(x, y, x+4, y+10, fill="#000000", width=0)
            self.flicker_pixels.append(pixel)
    ...
    def start_flicker(self):
        def flick():
            ...
            for px in self.flicker_pixels:
                self.flicker.itemconfig(px, fill=random.choice(tones))
            self.root.after(150, flick)
        flick()

    def stop_flicker(self):
        for px in self.flicker_pixels:
            self.flicker.itemconfig(px, fill="#000000")
    
    def start_key_selection(self):
        if self.listening:
            return
        self.listening = True
        self.select_btn.config(text="[ BUTTON it!... ]", bg="#00ff41", fg="#0d0d0d", state='disabled')
        self.key_label.config(text="WARTE...", fg="#00ff41")
        ...
        def on_key_press(key):
            if self.listening:
                ...
                self.root.after(0, lambda: self.key_label.config(text=f"BUTTONED: {key_name.upper()}", fg="#00ff41"))
                self.root.after(0, lambda: self.status_label.config(text="BUTTOONED!", fg="#00ff41"))
                self.root.after(0, lambda: self.select_btn.config(text="[ BUTTON ]", bg="#1a1a1a", fg="#00ff41", state='normal'))
                self.listening = False
        ...
        def on_mouse_click(x, y, button, pressed):
            if self.listening and pressed:
                ...
                self.root.after(0, lambda: self.key_label.config(text=f"BUTTONED: {btn_name}", fg="#00ff41"))
                self.root.after(0, lambda: self.status_label.config(text="BUTTOONED!", fg="#00ff41"))
                self.root.after(0, lambda: self.select_btn.config(text="[ BUTTON ]", bg="#1a1a1a", fg="#00ff41", state='normal'))
                self.listening = False
                self.key_listener.stop()
    ...
    def start_firing(self):
        if self.selected_key is None:
            self.status_label.config(text="KEIN BUTTON!", fg="#ff0000")
            return
        
        if self.delay_threshold > 0:
            self.status_label.config(text=f"WARTE {self.delay_threshold}s...", fg="#ffaa00")
            time.sleep(self.delay_threshold)
        
        self.firing = True
        self.status_label.config(text="BUTTOONING!!!", fg="#00ff41")
        self.start_flicker()
    ...
    def stop_firing(self):
        self.firing = False
        self.status_label.config(text="STOPPOONED", fg="#00ff41")
        self.stop_flicker()
        ...
        while self.firing and self.running:
            try:
                if self.is_mouse_button:
                    if self.hold_mode:
                        self.mouse_controller.press(self.selected_key)
                        time.sleep(1.0 / self.fire_rate)
                        self.mouse_controller.release(self.selected_key)
                    else:
                        self.mouse_controller.click(self.selected_key)
                else:
                    if self.hold_mode:
                        self.keyboard_controller.press(self.selected_key)
                        time.sleep(1.0 / self.fire_rate)
                        self.keyboard_controller.release(self.selected_key)
                    else:
                        self.keyboard_controller.press(self.selected_key)
                        self.keyboard_controller.release(self.selected_key)
...
def start_listener():
    with keyboard.GlobalHotKeys({"<alt>+^": on_activate, "<alt_gr>+^": on_activate}) as h:
        h.join()

kb = threading.Thread(target=start_listener, daemon=True)
kb.start()
